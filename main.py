    ...
    try:
        ...
        def try_open_file(path):
            try:
                ...
                print(f"AUTO OPEN ERROR: {ex}")
            return False

        def notify_export_success(label, path):
            opened = try_open_file(path)
            msg = f"{label}: {os.path.basename(path)}"
            if opened:
                msg += " | ouverture automatique lancée"
            else:
                msg += f" | chemin: {path}"
            ...
            try:
                ...
                if not page.logged_in and page.view == "LOGIN":
                    ...
                    screen = safe_screen(
                        ...
                        ft.Column([
                            err_desc,
                            ft.ElevatedButton("AJOUTER PHOTO PROBLÈME", icon=ft.Icons.CAMERA_ALT, on_click=lambda _: pick_img("ERR"), bgcolor="red900"),
                        ]),
                        ft.Column([
                            sol_desc,
                            ft.ElevatedButton("AJOUTER PHOTO SOLUTION", icon=ft.Icons.CAMERA_ALT, on_click=lambda _: pick_img("SOL"), bgcolor="green900"),
                        ]),
                        piec,
                        spare_price_in,
                    ...
                    def load_press_current_mold():
                        try:
                            ...
                            res = supabase.table("press_current_mold").select("*").eq("presse", p_dd.value).limit(1).execute()
                            if res.data:
                                data = res.data[0]
                                o_m.value = data.get("mold_name", "") or ""
                                apply_params(data)
                                info_txt.value = "Paramètres du moule sortant chargés."
                            else:
                                o_m.value = ""
                                clear_params()
                    ...
                    def load_mold_params_by_name():
                        try:
                            ...
                            if mold_name in MODEL_PARAMS:
                                apply_params(MODEL_PARAMS[mold_name])
                            ...
                            if db_res.data:
                                apply_params(db_res.data[0])
                                info_txt.value = "Paramètres récupérés depuis la base."
                            else:
                                clear_params()
                                info_txt.value = "Aucun paramètre trouvé. Saisissez-les puis validez pour les enregistrer."
                            page.update()
                        except Exception as ex:
                    ...
                    def save_m(e):
                        try:
                            if not p_dd.value:
                                page.snack_bar = ft.SnackBar(ft.Text("Choisissez une presse."), open=True)
                                page.update()
                                return
                            mold_name = (n_m.value or "").strip().upper()
                            if not mold_name:
                                page.snack_bar = ft.SnackBar(ft.Text("Entrez le nouveau moule."), open=True)
                                page.update()
                                return
                            ...
                            params_payload = {
                                ...
                                "mpa": float(mpa.value) if str(mpa.value).strip() not in ["", "None"] else None,
                                "cycle": int(float(cycle.value)) if str(cycle.value).strip() not in ["", "None"] else None,
                                "temps": temps.value,
                                "updated_by": page.display_name,
                                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            }
                            supabase.table("mold_parameters").upsert(params_payload, on_conflict="mold_name").execute()
                            current_payload = dict(params_payload)
                            current_payload.pop("mold_name", None)
                            current_payload.update({
                                "presse": p_dd.value,
                                "mold_name": mold_name,
                            })
                            supabase.table("press_current_mold").upsert(current_payload, on_conflict="presse").execute()
                            ...
                            page.snack_bar = ft.SnackBar(ft.Text("Changement moule enregistré avec succès."), open=True)
                            page.update()
                            ch_v("HOME")
                        except Exception as ex:
                            page.snack_bar = ft.SnackBar(ft.Text(f"Erreur: {ex}"), open=True)
                            page.update()
        ...
        def export_routines_excel(e=None):
            try:
                ...
                notify_export_success("Export Excel créé", filename)
            except Exception as ex:
        ...
        def export_excel(e=None):
            try:
                ...
                filename = os.path.join(get_downloads_dir(), base_name)
                df.to_excel(filename, index=False)
                notify_export_success("Excel exporté", filename)
            except Exception as ex:
                page.snack_bar = ft.SnackBar(ft.Text(f"Erreur export: {ex}"))
                page.snack_bar.open = True
                page.update()
        ...
        def export_inventory_excel(e=None):
            try:
                ...
                filename = os.path.join(get_downloads_dir(), base_name)
                df.to_excel(filename, index=False)
                notify_export_success("Inventaire exporté", filename)
            except Exception as ex:
                page.snack_bar = ft.SnackBar(ft.Text(f"Erreur export: {ex}"))
                page.snack_bar.open = True
                page.update()
