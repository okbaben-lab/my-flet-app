    ...
    try:
        ...
        def refresh():
            ...
            if not page.logged_in and page.view == "LOGIN":
                ...
                import threading  # ADD THIS AT TOP (no removal)
...
def login(e):
    ...
    def do_login():
        try:
            if supabase is None:
                raise Exception("Supabase not initialized (check INTERNET permission)")
            ...
            login_error.value = f"Erreur: {str(ex)}"
            login_error.visible = True
    ...
    threading.Thread(target=do_login).start()
                ...
                def open_stock_dialog(item):
                    ...
                    dlg = ft.AlertDialog(
                        ...
                        actions=[ft.TextButton("Annuler", on_click=lambda _: (setattr(dlg, "open", False), page.update())), 
                ...
                def save_i(e):
                    ...
                    supabase.table("inters").insert({
                        ...
                        "systeme": sys_dd.value, "sous_ens": s_ens.value, "error_desc": err_desc.value, 
                        "solution_desc": sol_desc.value, "pieces": piec.value, "type_m": m_type.value, 
                        "photo_err": final_err_img_url, "photo_sol": final_sol_img_url, "error_source": error_source.value, 
                        "error_other": error_other_desc.value, "user": page.display_name, 
                        "mold_ref": mold_ref_field.value, "spare_price": spare_price_in.value
                    }).execute()
                    page.photo_err_path = ""; page.photo_sol_path = ""
                    ch_v("HOME")
                ...
                def on_p_change(e):
                    ...
                    o_m.value = res.data[0]['new_m'] if res.data else "Vide"; page.update()
        ...
        def generate_weekly_pdf(e=None):
            try:
                ...
                pdf.cell(40, 10, "Date/Heure", 1); pdf.cell(40, 10, "Machine", 1); pdf.cell(30, 10, "Opérateur", 1); pdf.cell(80, 10, "Points de contrôle", 1); pdf.ln()
