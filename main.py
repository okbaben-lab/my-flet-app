        ...
        def capture_photo(target):
            ...
            try:
                if hasattr(page, "get_media") and hasattr(ft, "MediaSource"):
                    ...
                    raise Exception("Capture photo non supportée")
            except Exception as ex:
                print(f"Camera not available: {ex}")
                page.snack_bar = ft.SnackBar(
                    ft.Text("La caméra n'est pas supportée sur cette version APK. Mettez à jour Flet APK.")
                )
                page.snack_bar.open = True
                page.update()
                ...
                if not page.logged_in and page.view == "LOGIN":
                    ...
                    def save_m(e):
                        try:
                            ...
                            supabase.table("mold_parameters").upsert(payload, on_conflict="mold_name").execute()
                            supabase.table("press_current_mold").upsert({
                                "presse": p_dd.value,
                                **payload,
                            }, on_conflict="presse").execute()
