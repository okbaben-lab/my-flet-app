    ...
    try:
        ...
        def on_file_result(e):
            try:
                ...
                page.last_action_status = f"Erreur galerie: {ex}"
                page.snack_bar = ft.SnackBar(ft.Text(page.last_action_status))
                page.snack_bar.open = True
                page.update()
        ...
        async def capture_photo(target):
            await pick_from_gallery(target)
            ...
            try:
                ...
                if not page.logged_in and page.view == "LOGIN":
                            ...
                            [
                                ft.Row(
                                    [
                                        ft.Column(
                                            [
                                                ft.Text("VERSION: OKBA-2026-DOWNLOAD", color="yellow", size=12),
                    ...
                    screen = safe_screen(
                        ...
                        p_qty,
                        p_urg,
                        ft.ElevatedButton("ATTACHER PHOTO PIÈCE (GALERIE)", icon=ft.Icons.PHOTO_LIBRARY, on_click=lambda e: page.run_task(pick_from_gallery, "PART")),
                    ...
                    photo_err_manual = ft.TextField(label="Lien / chemin photo problème (si caméra indisponible)", width=320)
                    photo_sol_manual = ft.TextField(label="Lien / chemin photo solution (si caméra indisponible)", width=320)
                    ...
                    def save_i(e):
                        try:
                            ...
                            final_err_img_url = resolve_photo_source(page.photo_err_path or photo_err_manual.value, "errors")
                            final_sol_img_url = resolve_photo_source(page.photo_sol_path or photo_sol_manual.value, "solutions")
                    ...
                    screen = safe_screen(
                        ...
                        ft.Text(page.last_action_status or "Photo: utilisez la galerie pour attacher une image.", color="yellow"),
                        dem,
                        sys_dd,
                        mold_ref_field,
                        s_ens,
                        ...
                        ft.Column([
                            err_desc,
                            ft.ElevatedButton("ATTACHER PHOTO PROBLÈME (GALERIE)", icon=ft.Icons.PHOTO_LIBRARY, on_click=lambda e: page.run_task(pick_from_gallery, "ERR"), bgcolor="red900"),
                            photo_err_manual,
                        ]),
                        ft.Column([
                            sol_desc,
                            ft.ElevatedButton("ATTACHER PHOTO SOLUTION (GALERIE)", icon=ft.Icons.PHOTO_LIBRARY, on_click=lambda e: page.run_task(pick_from_gallery, "SOL"), bgcolor="green900"),
                            photo_sol_manual,
                        ]),
                        piec,
                        spare_price_in,
                    ...
                    p_dd = ft.Dropdown(label="Presse", width=320, options=[ft.dropdown.Option(f"Presse {i+1}") for i in range(11)])
                    ...
                    def fill_fields(data, mold_name=""):
                        o_m.value = mold_name if mold_name is not None else o_m.value
                        ...
                        try:
                            ...
                            if res.data:
                                ...
                                if hist.data:
                                    h = hist.data[0]
                                    data = {
                                        "mold_name": h.get("new_m", "") or "",
                                        "modele": h.get("modele", "") or "",
                                        "matiere": h.get("matiere", "A018") or "A018",
                                        "mpa": h.get("mpa", "") or "",
                                        "cycle": h.get("cycle", "") or "",
                                        "temps": h.get("temps", "") or "",
                                    }

                            if data:
                                o_m.value = str(data.get("mold_name", "") or "")
                                fill_fields(data)
                            ...
                            print(f"PRESS CURRENT LOAD ERROR: {ex}")
                    ...
                    def load_mold_params_by_name():
                        try:
                            ...
                            if mold_name in MODEL_PARAMS:
                                fill_fields(MODEL_PARAMS[mold_name])
                            ...
                            print(f"LOAD MOLD PARAMS ERROR: {ex}")
                    ...
                    def save_m(e):
                        try:
                            ...
                            info_txt.value = "Enregistrement du moule..."
                            page.last_action_status = "Enregistrement du moule..."
                            page.snack_bar = ft.SnackBar(ft.Text("Enregistrement du moule..."))
                            page.snack_bar.open = True
                            page.update()
                            ...
                            payload = {
                                ...
                                "mpa": float(mpa.value) if str(mpa.value).strip() else None,
                                "cycle": int(float(cycle.value)) if str(cycle.value).strip() else None,
                                "temps": temps.value,
                                "updated_by": page.display_name,
                                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            }

                            # Save mold parameters without relying on upsert support
                            existing_param = (
                            ...
                            if existing_param.data:
                                supabase.table("mold_parameters").update(payload).eq("id", existing_param.data[0]["id"]).execute()
                            else:
                            ...
                            if existing_press.data:
                                supabase.table("press_current_mold").update(press_payload).eq("id", existing_press.data[0]["id"]).execute()
                            else:
                                supabase.table("press_current_mold").insert(press_payload).execute()
                            ...
                            info_txt.value = "✅ Changement moule enregistré."
                            page.last_action_status = "✅ Changement moule enregistré."
                            o_m.value = mold_name
                            info_txt.value = "✅ Changement moule enregistré."
                            o_m.value = mold_name
                            info_txt.value = "✅ Changement moule enregistré."
                            ...
                            print(f"SAVE MOLD ERROR: {ex}")
                            info_txt.value = f"Erreur moule: {ex}"
                            page.last_action_status = f"Erreur moule: {ex}"
